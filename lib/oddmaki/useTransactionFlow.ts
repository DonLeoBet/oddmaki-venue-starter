...
/**
 * Translate raw on-chain / wallet errors into messages a non-technical
 * trader can act on. Falls back to the original message when unmapped.
 */
export function humanizeFlowError(err: unknown): string {
  const raw = err instanceof Error ? err.message : String(err);

  if (/TransferFromFailed|ERC20: transfer amount exceeds balance/i.test(raw)) {
    return "Insufficient USDC balance — add funds and try again.";
  }
  if (/User rejected|User denied|rejected the request/i.test(raw)) {
    return "Transaction rejected in wallet.";
  }
  if (/insufficient funds for gas/i.test(raw)) {
    return "Not enough ETH to pay for gas on Base.";
  }
  if (/NoLiquidityAvailable/i.test(raw)) {
    return "No matching liquidity at this price. Try a limit order.";
  }
    ...
    async (startIndex: number) => {
      ...
      for (let i = startIndex; i < steps.length; i++) {
        ...
        try {
          ...
          const message = humanizeFlowError(err);
