...
import {
  isLeagueVisibleForBrand,
  isMarketTypeVisibleForBrand,
} from "@/config/brandMarkets";
...
/** GlazenBol: /glazenbol/[marketTypeSlug]/[leagueSlug] (market-first). */
...
export default async function GlazenBolCategoryPage({ params }: PageProps) {
  const { leagueSlug, marketTypeSlug } = await params;
  const brandId = resolveBrandId(BRAND_CONFIG.id);
  const marketType = marketTypeFromSlug(brandId, marketTypeSlug);
  const league = leagueSlugFromRoute(brandId, leagueSlug);
