...
interface PriceInfoHeaderProps {
  ...
  resolved?: boolean;
  /**
   * Asset price captured at the market's closeTime (from Pyth). Shown once the
   * market has expired but is not yet resolved, so the displayed price reflects
   * how the market ended rather than the still-moving live price.
   */
  closePrice?: number;
  /** `true` when now is past closeTime but the market hasn't resolved yet. */
  expired?: boolean;
  /**
...
export function PriceInfoHeader({
  ...
  const showClose = !resolved && expired;
