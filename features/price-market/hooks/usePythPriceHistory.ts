...
export function usePythPriceHistory(feedId: string | undefined) {
  ...
  // Append a new data point only when publishTime moves strictly forward.
  // Pyth's "latest" endpoint can return a VAA whose publishTime is a second
  // older than a prior poll; appending it would make the series non-monotonic
