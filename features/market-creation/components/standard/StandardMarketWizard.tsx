...
import { StepStandardTrading } from "./StepStandardTrading";
...
function isStepValid(
  ...
  switch (step) {
    ...
    case "trading": {
      if (form.liquidityMode !== "pool") return true;
      // Pool markets require a close time and, when scheduled, an open time.
      // Close must be strictly after the effective open time.
      if (!form.closeDatetime.trim()) return false;
      if (form.openMode === "scheduled" && !form.openDatetime.trim())
        return false;
      const openMs =
        form.openMode === "scheduled" && form.openDatetime
          ? new Date(form.openDatetime).getTime()
          : Date.now();

      return new Date(form.closeDatetime).getTime() > openMs;
    }
    case "settings": {
  ...
  return (
    <>
      ...
      >
        ...
        {currentStep === "trading" && (
          <StepStandardTrading formData={formData} updateField={updateField} />
        )}
