...
/**
 * Liquidity mechanism for a market.
 * - `order-book`: central limit order book (CLOB). Default.
 * - `pool`: dynamic pari-mutuel (DPM) pool.
 */
export type LiquidityMode = "order-book" | "pool";

export interface LiquidityModeOption {
  id: LiquidityMode;
  label: string;
  description: string;
  finePrint: string;
...
export const LIQUIDITY_MODES: LiquidityModeOption[] = [
  {
    id: "order-book",
    label: "Order Book",
    description:
      "Traders match buy and sell orders. Use when a market has active traders or a market maker.",
    finePrint: "Central Limit Order Book",
  },
  {
    id: "pool",
    label: "Pool",
    description:
      "Traders put money into a shared pool and split it among winners. Stays liquid even with few traders. Use for new or quiet markets.",
    finePrint: "Dynamic Pari-Mutuel",
  },
];
...
export const STANDARD_WIZARD_STEPS: {
  ...
  },
  {
    ...
    label: "Trading",
    description:
      "Pick how the market trades. Order Book sets a tick size; Pool sets its open and close times.",
  },
  {
    id: "settings",
    number: 4,
    label: "Oracle",
    description: "Optional UMA reward boost, challenge period, and tags.",
  },
  {
    id: "access-control",
    number: 5,
...
export interface StandardMarketFormData {
  ...
  tickSize: TickSize;
  /** Pool only. `immediate` opens when the create tx mines (openTime = 0); `scheduled` opens at openDatetime. */
  openMode: "immediate" | "scheduled";
  /** Pool only. datetime-local string, used when openMode === "scheduled". */
  openDatetime: string;
  /** Pool only. datetime-local string. Required when liquidityMode === "pool". */
...
export type PriceWizardStep = "feed" | "window" | "meta" | "trading" | "review";
...
export const PRICE_WIZARD_STEPS: {
  ...
  },
  {
    ...
    number: 3,
    label: "Question",
    description: "Auto-generated title and description, and tags.",
  },
  {
    id: "trading",
    number: 4,
    label: "Trading",
    description:
      "Pick how the market trades. Order Book sets a tick size; Pool needs neither.",
  },
  {
    id: "review",
    number: 5,
