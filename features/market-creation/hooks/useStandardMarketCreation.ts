...
export function useStandardMarketCreation(
  ...
  const createMarket = useCallback(
    async (formData: StandardMarketFormData, signer: `0x${string}`) => {
      ...
      steps.push({
        id: "create-market",
        label: isPool ? "Create pool on-chain" : "Create market on-chain",
        execute: async () => {
          ...
          if (isPool) {
            // Pool (DPM): pari-mutuel. closeTime is required; openTime=0 means
            // "immediate" (contract opens at tx-mine time). Both unix seconds.
            const closeTimeUnix = Math.floor(
              new Date(formData.closeDatetime).getTime() / 1000,
            );
            const openTimeUnix =
              formData.openMode === "scheduled" && formData.openDatetime
                ? Math.floor(new Date(formData.openDatetime).getTime() / 1000)
                : 0;

            hash = await client.dpm.createMarket({
              venueId,
              outcomes,
              ...
              openTime: BigInt(openTimeUnix),
              closeTime: BigInt(closeTimeUnix),
              additionalReward,
              liveness: BigInt(formData.liveness),
              tags: formData.tags,
            });
          } else {
            hash = await client.market.createMarket({
              venueId,
              question: {
                title: formData.title,
                description: formData.description,
