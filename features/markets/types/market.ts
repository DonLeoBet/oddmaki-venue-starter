...
export interface Market {
  ...
  // Liquidity mode: true when this market trades via the DPM ("Pool") pool
  // rather than the CLOB. Carried by the unified-feed query.
  isDpmMarket?: boolean;
  // DPM pool overlay (null for CLOB markets). Carried by the unified-feed query
  // so cards can show implied odds (collateral split) without an extra call.
  dpmMarket?: {
    poolInitialized: boolean;
    totalCollateral: string;
    outcomes: { outcomeIndex: string; collateral: string }[];
  } | null;
